...
| `groupByDate(reports)` | `MonthlyReport[]` → `Map<date, MonthlyReport[]>` |
...
`aggregateReports(reports: MonthlyReport[]): AggregatedStats`

집계 항목: totalDays, totalSessions, totalToolCalls, totalProjects, toolUsageAggregated, taskTypeAggregated (percentage 포함), techStackFrequency (파일 단위 카운트), monthlyTrend, averageEvaluationScore, latestEvaluation, evaluationCategoryAverages
