...
| `monthly/` | 월간 일지 상세 페이지 전용 | 10개 섹션 컴포넌트 + `MonthlyHeader.tsx` |
| `monthly-list/` | 월간 일지 목록 페이지 전용 | `CalendarView.tsx` (월 선택기), `ListView.tsx` (groupByDate 후 날짜역순) |
...
UsageScoreCard (score, maxScore, categories, grade?, taskComplexity?, ...)
  ...
  └─ MonthlyDetailPage: size="sm", featured, title="활용도 평가", 단일 리포트
```
