...
| `useMonthlyReport(filename)` | `["report", filename]` | `MonthlyReport` | `enabled: !!filename` |
| `useAllReports()` | `["fileList"]` + 각 `["report", ...]` | `{ data: MonthlyReport[], isLoading, isError }` | useFileList + useQueries 병렬 |
...
/data/{location}/{name}.json
    ↓ useMonthlyReport("work/2026-02")
MonthlyReport → MonthlyDetailPage 렌더링
```
