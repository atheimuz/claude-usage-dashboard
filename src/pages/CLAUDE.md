...
| `MonthlyListPage` | `/monthly` | `useAllReports` | CalendarView (월 선택기), ListView |
| `MonthlyDetailPage` | `/monthly/:location/:name` | `useMonthlyReport` + `useFileList` | MonthlyHeader + 10개 섹션 컴포넌트 (monthly/ 참조) |
...
## MonthlyDetailPage URL 파라미터 검증

```ts
const LOCATION_PATTERN = /^[a-z0-9_-]+$/i   // location: "work", "side" 등
const NAME_PATTERN = /^\d{4}-\d{2}$/        // name: "2026-02"
```
