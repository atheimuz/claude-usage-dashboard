...
import { useMonthlyReport, useFileList } from "@/hooks/useReports";
...
import { UsageStyleSection } from "@/components/monthly/UsageStyleSection";
import { ClaudeUsageSection } from "@/components/monthly/ClaudeUsageSection";
import { PromptPatterns } from "@/components/monthly/PromptPatterns";
import { ToolStatsTable } from "@/components/shared/ToolStatsTable";
import { TaskTypeGrid } from "@/components/shared/TaskTypeGrid";
import { ErrorSummarySection } from "@/components/monthly/ErrorSummarySection";
import { LearningInsights } from "@/components/monthly/LearningInsights";
import { WorkflowPatterns } from "@/components/monthly/WorkflowPatterns";
import { ConfigChangesSection } from "@/components/monthly/ConfigChangesSection";

const LOCATION_PATTERN = /^[a-z0-9_-]+$/i;
const NAME_PATTERN = /^\d{4}-\d{2}$/;
...
export function MonthlyDetailPage() {
    ...
    const { data: report, isLoading, isError, refetch } = useMonthlyReport(filename);
            ...
            <div className="flex flex-col items-center justify-center py-20">
                {isError ? (
                    <Alert variant="destructive" className="max-w-md">
                        ...
                        <AlertDescription>해당 월간 일지를 불러올 수 없습니다.</AlertDescription>
                        <div className="mt-3 flex gap-2">
                            <Button size="sm" onClick={() => refetch()}>
                                다시 시도
                            </Button>
                            <Button size="sm" variant="outline" asChild>
                                <Link to="/monthly">목록으로</Link>
                            </Button>
                        </div>
                    </Alert>
                ) : (
                    <>
                        ...
                        <p className="text-lg font-semibold">해당 월의 일지가 없습니다.</p>
                        <Button className="mt-4" variant="outline" asChild>
                            <Link to="/monthly">목록으로 돌아가기</Link>
                        </Button>
                    </>
                )}
            </div>
    ...
    return (
        <div className="space-y-6">
            <MonthlyHeader report={report} prevFilename={prev} nextFilename={next} />
