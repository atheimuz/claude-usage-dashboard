...
│   ├── monthly-list.spec.ts   # 월간 일지 목록
│   ├── monthly-detail.spec.ts # 월간 일지 상세 (17개 시나리오)
...
│   ├── home.page.ts
│   ├── monthly-list.page.ts
│   └── monthly-detail.page.ts
└── mocks/
...
| Mock 파일 | `mocks/*.mock.ts` | 타입 import (`MonthlyReport` 등), 변수명 (`monthlyTrend` 등), 주석 |
